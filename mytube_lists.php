...
class mytubeLists {
    ...
    function mytubeLists( $path='uploads', $value = null, $selected='', $size = 1, $emptyselect = 0, $type = 0, $prefix='', $suffix='' ) {
        $this -> value = $value;
     ...
     *
     * @param $dirname The directory for which you want to have the contents as an array
     *
     * @return array
     */
    ...
    function &getListTypeAsArray( $dirname, $type='', $prefix='', $noselection = 1 ) {
        ...
        switch ( trim( $type ) ) {
            case 'images':
                $types = '[.gif|.jpg|.png]';
                if ( $noselection )
                    $filelist[''] = _AM_MYTUBE_NOIMAGE;
                break;
            case 'media':
                $types = '[.aac|.flv|.mp3|.mp4|.swf]';
                if ( $noselection )
                    $filelist[''] = _AM_MYTUBE_NOVIDEO;
                break;
            case 'html':
                $types = '[.htm|.html|.xhtml|.php|.php3|.phtml|.txt]';
                if ( $noselection )
                    $filelist[''] = _AM_MYTUBE_NOSELECT;
                break;
            default:
                $types = '';
                if ( $noselection )
                    $filelist[''] = _AM_MYTUBE_NOFILESELECT;
                break;
        }
