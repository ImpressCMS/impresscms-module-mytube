...
if ( is_array( $arr ) > 0 && !$list && !$selectdate ) {
	$scount = 1;
	foreach( $arr as $ele ) {
		if ( mytube_checkgroups( $ele['cid'] ) == false ) { continue; }
		$sub_arr = array();
		...
		foreach( $sub_arr as $sub_ele ) {
			$hassubitems = mytube_getTotalItems( $sub_ele['cid'], 1 ); // Subitem file count
		...
		$xoopsTpl -> append( 'subcategories', array(
			'title' => $mytubemyts -> htmlSpecialCharsStrip( $ele['title'] ),
			'id' => $ele['cid'],
