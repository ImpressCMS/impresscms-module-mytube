...
switch ( strtolower( $op ) ) {
    ...
    default:
        ...
        $mytree = new XoopsTree( $xoopsDB -> prefix( 'xoopstube_mod' ), "requestid", 0 );
        ...
        if ( $totalmodrequests > 0 ) {
            while ( $video_arr = $xoopsDB -> fetchArray( $result ) ) {
                ...
                $title = trim( $path );
                $submitter = xoops_getLinkedUnameFromId( $video_arr['modifysubmitter'] );;
                $requestdate = formatTimestamp( $video_arr['requestdate'], $xoopsModuleConfig['dateformatadmin'] );
